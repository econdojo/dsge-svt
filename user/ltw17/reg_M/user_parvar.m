...
para = {
           'gam100 ', 0.28, 0, 1e5, 'N', 0.40, 0.05     % [1] 100*s.s. growth rate of technology
           'xi     ', 1.54, 0, 1e5, 'G', 2.00, 0.50     % [2] inverse Frisch labor elasticity
           'theta  ', 0.95, 0, 1,   'B', 0.50, 0.20     % [3] habit formation
           'alpha_g',-.56,-1.75,1.75,'U',-1.75, 1.75    % [4] subs. of private/public consumption
           'psi    ', 0.17, 0, 1,   'B', 0.60, 0.15     % [5] capital utilization
           's      ', 4.29, 0, 1e5, 'N', 6.00, 1.50     % [6] investment adjustment cost
           'omega_p', 0.83, 0, 1,   'B', 0.50, 0.20     % [7] price stickiness
           'omega_w', 0.72, 0, 1,   'B', 0.50, 0.20     % [8] wage stickiness
           'chi_p  ', 0.09, 0, 1,   'B', 0.50, 0.20     % [9] price partial indexation
           'chi_w  ', 0.07, 0, 1,   'B', 0.50, 0.20     % [10] wage partial indexation
           'phi_p  ', 1.11, 0, 1e5, 'N', 1.50, 0.20     % [11] interest rate response to inflation
           'phi_y  ', 0.21, 0, 1e5, 'N', 0.125,0.05     % [12] interest rate response to output
           'rho_r  ', 0.76, 0, 1,   'B', 0.50, 0.20     % [13] interest rate lagged response
           'gam_g  ', 0.16,-5, 1e5, 'N', 0.15, 0.10     % [14] G response to debt
           'gam_z  ', 0.19,-5, 1e5, 'N', 0.15, 0.10     % [15] Z response to debt
           'rho_g  ', 0.98, 0, 1,   'B', 0.50, 0.20     % [16] G lagged response
           'rhos_a ', 0.22, 0, 1,   'B', 0.50, 0.20     % [17] technology shock AR coeff.
           'rhos_b ', 0.18, 0, 1,   'B', 0.50, 0.20     % [18] preference shock AR coeff.
           'rhos_i ', 0.44, 0, 1,   'B', 0.50, 0.20     % [19] investment shock AR coeff.
           'rhos_p ', 0.69, 0, 1,   'B', 0.50, 0.20     % [20] price markup shock AR coeff.
           'rhos_w ', 0.27, 0, 1,   'B', 0.50, 0.20     % [21] wage markup shock AR coeff.
           'rhos_m ', 0.57, 0, 1,   'B', 0.50, 0.15     % [22] monetary shock AR coeff.
           'rhos_g ', 0.30, 0, 1,   'B', 0.50, 0.15     % [23] G shock AR coeff.
%            'sigma_a', 1.17, 0, 1e5, 'I1',2.006,0.057    % [24] 100*technology shock s.d.
%            'sigma_b',12.03, 0, 1e5, 'I1',2.006,0.057    % [25] 100*preference shock s.d.
%            'sigma_i', 1.28, 0, 1e5, 'I1',2.006,0.057    % [26] 100*investment shock s.d.
%            'sigma_p', 0.14, 0, 1e5, 'I1',2.006,0.057    % [27] 100*price markup shock s.d.
%            'sigma_w', 0.23, 0, 1e5, 'I1',2.006,0.057    % [28] 100*wage markup shock s.d.
%            'sigma_m', 0.23, 0, 1e5, 'I1',2.006,0.057    % [29] 100*monetary shock s.d.
%            'sigma_g', 2.04, 0, 1e5, 'I1',2.006,0.057    % [30] 100*G shock s.d.
%            'sigma_z', 1.90, 0, 1e5, 'I1',2.006,0.057    % [31] 100*Z shock s.d.
           'lbar   ',469.81,0, 1e5, 'N', 468,  5.00     % [32] s.s. hours worked
           'pbar   ', 0.74, 0, 1e5, 'N', 0.75, 0.25     % [33] s.s. inflation
          };
...
ssp = {
           ...
           'rho_z'      % Z lagged response
           'rho_c'      % consumption tax lagged response
           'rhos_z'     % Z shock AR coeff.
           % Steady state
           'rho'
           'gam'
           'expg'
