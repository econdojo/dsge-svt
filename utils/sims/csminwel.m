function [fh,xh,gh,H,itct,fcount,retcodeh] = csminwel(fcn,x0,H0,grad,crit,nit,nodisp,varargin)
...
% nodisp:deactivate disp.m if it equals one; added by Fei Tan on 12/15/2016
...
if nit==0     % added by Fei Tan (2/15/2017)
    fh=[]; xh=x0; gh=[]; H=H0; itct=[]; fcount=[]; retcodeh=[];
    return
end
[nx,no]=size(x0);
nx=max(nx,no);
...
%if f0 > 1e100, disp('Bad initial parameter.'), return, end
if f0>1e100   % added by Fei Tan (1/27/2017)
    x0 = varargin{4}.para(varargin{1},1)';
    H0 = eye(length(x0))*1e-2;
    f0 = feval(fcn,x0,varargin{:});
end
if NumGrad
   if isempty(grad)
      [g, badg] = numgrad(fcn,x0,nodisp,varargin{:});
      %ARGLIST
      %[g badg] = numgrad(fcn,x0,nodisp,P1,P2,P3,P4,P5,P6,P7,P8,P9,P10,P11,P12,P13);
   else
      badg=any(find(grad==0));
      g=grad;
   end
   %numgrad(fcn,x0,nodisp,P1,P2,P3,P4);
else
...
x=x0;
f=f0;
fh=f0;   % added by Fei Tan, 1/1/2017
H=H0;
%cliff=0;
while ~done
   g1=[]; g2=[]; g3=[];
   ...
   if ~nodisp
      ...
      fprintf('f at the beginning of new iteration, %20.10f\n',f)
      %-----------Comment out this line if the x vector is long----------------
      disp([sprintf('x = ') sprintf('%15.8g %15.8g %15.8g %15.8g\n',x)]);
   end
   %-------------------------
   itct=itct+1;
   [f1, x1, fc, retcode1] = csminit(fcn,x,f,g,badg,H,nodisp,varargin{:});
   %ARGLIST
   %[f1 x1 fc retcode1] = csminit(fcn,x,f,g,badg,H,nodisp,P1,P2,P3,P4,P5,P6,P7,...
   ...
   if retcode1 ~= 1
      if retcode1==2 || retcode1==4
         ...
         if NumGrad
            [g1, badg1] = numgrad(fcn, x1,nodisp,varargin{:});
            %ARGLIST
            %[g1 badg1] = numgrad(fcn, x1,nodisp,P1,P2,P3,P4,P5,P6,P7,P8,P9,...
            %                P10,P11,P12,P13);
         else
      ...
      if wall1 && (length(H) > 1)% 
         ...
         [f2, x2, fc, retcode2] = csminit(fcn,x,f,g,badg,Hcliff,nodisp,varargin{:});
         %ARGLIST
         %[f2 x2 fc retcode2] = csminit(fcn,x,f,g,badg,Hcliff,nodisp,P1,P2,P3,P4,...
         ...
         if  f2 < f
            if retcode2==2 || retcode2==4
               ...
               if NumGrad
                  [g2, badg2] = numgrad(fcn, x2,nodisp,varargin{:});
                  %ARGLIST
                  %[g2 badg2] = numgrad(fcn, x2,nodisp,P1,P2,P3,P4,P5,P6,P7,P8,...
                  %      P9,P10,P11,P12,P13);
               else
            ...
            if wall2
               ...
               if norm(x2-x1) < 1e-13
                  ...
                  [f3, x3, fc, retcode3] = csminit(fcn,x,f,gcliff,0,eye(nx),nodisp,varargin{:});
                  %ARGLIST
                  %[f3 x3 fc retcode3] = csminit(fcn,x,f,gcliff,0,eye(nx),nodisp,P1,P2,P3,...
                  %         P4,P5,P6,P7,P8,...
                  %      P9,P10,P11,P12,P13);
                  fcount = fcount+fc; % put by Jinill
                  if retcode3==2 || retcode3==4
                     ...
                     if NumGrad
                        [g3, badg3] = numgrad(fcn, x3,nodisp,varargin{:});
                        %ARGLIST
                        %[g3 badg3] = numgrad(fcn, x3,nodisp,P1,P2,P3,P4,P5,P6,P7,P8,...
                        %                        P9,P10,P11,P12,P13);
                     else
   ...
   elseif f1 < f - crit && badg1==0
      ...
      if nogh
         if NumGrad
            [gh , ~] = numgrad(fcn,xh,nodisp,varargin{:});
         else
            [gh , ~] = feval(grad, xh,varargin{:});
         end
      end
      badgh=1;
   end
   ...
   stuck = (abs(fh-f) < crit);
   if (~badg)&&(~badgh)&&(~stuck)
      H = bfgsi(H,gh-g,xh-x,nodisp);
   end
   if ~nodisp
      disp('----')
      fprintf('Improvement on iteration %d = %18.9f\n',itct,f-fh)
   end
   % if Verbose
      if itct > nit
         if ~nodisp
