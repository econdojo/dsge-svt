...
if ~zxz
   ...
   % Line 44 modified (by Fei Tan, 12/15/2016) as below:
   [a,b,q,z]=qzdiv_mex(div,a,b,q,z);
end
gev=[diag(a) diag(b)];
if zxz
...
G1=G0\G1;
usix=n-nunstab+1:n;
C=G0\[tmat*q*c;(a(usix,usix)-b(usix,usix))\q2*c];
...
loose = G0\[etawt1 * (eye(neta) - veta * veta');zeros(nunstab, neta)];
