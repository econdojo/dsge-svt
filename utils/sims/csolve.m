function [x,rc] = csolve(FUN,x,gradfun,crit,itmax,verbose,varargin)
%function [x,rc] = csolve(FUN,x,gradfun,crit,itmax,verbose,varargin)
...
% verbose:  if this is set to zero, all screen output is suppressed; added by Fei Tan on 5/23/2017
...
while ~done
   ...
   if randomize
      if(verbose),disp('Random Search');end
   ...
   itct=itct+1;
   if(verbose)
      fprintf('itct %d, af %g, lambda %g, rc %g\n',itct,afmin,lambdamin,rc);
      fprintf('   x  %10g\n',xmin);
      fprintf('   f  %10g\n',fmin);
   end
   x=xmin;
