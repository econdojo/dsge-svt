function se = DistSmoother(Y,SSR,fs,Omega_fs)
...
if size(SSR.Sigma_e,2)==1
    Sigma_e = repmat(SSR.Sigma_e,1,T);      % constant
else
    Sigma_e = SSR.Sigma_e;                  % time-varying
end
...
for t = 1:T
    ...
    ps = SSR.C+SSR.G*fs;
    Omega_ps = SSR.G*Omega_fs*SSR.G'+SSR.M*diag(Sigma_e(:,t))*SSR.M';
